...
class CtrlTextField extends PureComponent {
    ...
    getErrorMsg(value) {
        ...
        if (type === inputType.NUMERIC || type === inputType.CURRENCY) {
    ...
    handleInputOnChange(evt) {
        ...
        if (value !== ''
            && (type === inputType.NUMERIC || type === inputType.CURRENCY)
    ...
    render() {
        ...
        let helperText = hasError ? errorMsg : EMPTY_ERROR;
        let shownValue = value;

        if (!hasError && value && type === inputType.CURRENCY) {
            const currencyFormat = toMoney({ decimals, number: value });
            helperText = disabled ? null : currencyFormat;
            shownValue = disabled ? currencyFormat : value;
        } else if (!hasError && Boolean(bottomText)) {
...
CtrlTextField.propTypes = {
    ...
    textTranform: PropTypes.oneOf([...Object.values(transformType)]),
